...
def autoeliminador():
	"""Funcion que autoelimina usuarios ya vencidos, sin hacerlos manualmente """

	conct = sqlite3.connect("recoveri.db")
	...
	formato_hora = f"{hoy.year}/{hoy.month}/{hoy.day}"

	#print("Hoy=>",formato_hora[5:6])

	datos = cursor.execute("SELECT expiracion FROM reiniciado").fetchall()

	for x in datos:
		if int(x[0][7:9]) ==  int(formato_hora[7:9]) or int(x[0][7:9]) < int(formato_hora[7:9]):
			if int(x[0][5:6]) ==  int(formato_hora[5:6]):
				cursor.execute("DELETE FROM reiniciado Where expiracion ='{}'".format(x[0]))
				print(yellow + x[0],red + "=> Vencido ==[ELIMINADO]==")
			else:
				print(green + x[0],green + "=> Valido")
...
def find_ip():
	...
	if os.path.exists("file_ip.txt"):
		with open("file_ip.txt","r") as file:
			data = file.read()
			new = data.split(",")
			ip_c = new[0]
			ip_mvb = new[1]
		...
		while True:
			print(yellow + "========================== Llena los datos ===========================")
			ipclaro = input("Ip del servidor para CLARO: ")
			ipmovistar = input("Ip del servidor de Movistar y Bitel: ")

			if len(ipclaro) == 0 and len(ipmovistar) == 0:
				print(red + "\n\t ------ No has llendo la suficiente informacion ------")
				time.sleep(1)
				os.system(cleaner)
				return find_ip()
			else: 
				with open("file_ip.txt","w") as f:
					f.write(ipclaro +","+ipmovistar)
					break
...
def menu_Actualizador():
	...
	if op == 1:
		...
		c.ghostbusters(green + "Nos Vemos ADMIN!!!")
		sys.exit()
